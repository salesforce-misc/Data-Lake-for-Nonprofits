...
export type PartialS3EventListing = Pick<S3ListingEvent, "Key"> &
  Partial<S3ListingEvent>;
...
export const SchemaFieldType = {
  ADDRESS: "address", // UNSUPPORTED
  ANY: "anyType",
  CALCULATED: "calculated", // UNSUPPORTED
  COMPLEX_VALUE: "complexvalue", // UNSUPPORTED
  COMBOBOX: "combobox",
  CURRENCY: "currency",
  DATA_CATEGORY_GROUP_REFERENCE: "datacategorygroupreference", // UNSUPPORTED
  EMAIL: "email",
  ENCRYPTED_STRING: "encryptedstring",
  ID: "id",
  JUNCTION_ID_LIST: "junctionidlist", // UNSUPPORTED
  LOCATION: "location", // UNSUPPORTED
  MASTER_RECORD: "masterrecord", // UNSUPPORTED
...
export const SchemaFieldTypeReverse: SchemaFieldTypeReverse<SchemaFieldTypeOf> =
  invert(SchemaFieldType) as SchemaFieldTypeReverse<SchemaFieldTypeOf>;
export const UNSUPPORTED_FIELDS = [
  SchemaFieldType.ADDRESS,
  SchemaFieldType.CALCULATED,
  SchemaFieldType.COMPLEX_VALUE,
  SchemaFieldType.DATA_CATEGORY_GROUP_REFERENCE,
  SchemaFieldType.JUNCTION_ID_LIST,
  SchemaFieldType.MASTER_RECORD,
  SchemaFieldType.LOCATION,
  SchemaFieldType.BYTE,
];
export const IDENTIFIER_TYPE = SchemaFieldType.ID;

export function toSchemaFieldType(
...
export function flowName(
  schemaOrSchemaName: Schema | string,
  installationId?: string
): string {
...
export function partialS3EventFrom(
  schemaName: string,
  installationId: string
): PartialS3EventListing {
  return { Key: `schemas/${schemaName}.${installationId}.schema.json` };
}

export function extractSchemaName(
  event: PartialS3EventListing | S3ListingEvent
): [string, string] {
  const [schemaName, installationId] = event.Key.replace(
    /schemas\/|\.schema\.json/gi,
    ""
  ).split(".");
...
export function buildEntityMap(
  entities: { [k in string]: ConnectorEntity[] },
